...
// added feature for EELS quantification whereby the output tabbed column files adopt the root name of
// their parent EELS spectrum. This is accomplished by making the EELS spectrum the front figure before
// execting script.

// 1. User no loger has option to export all or top; all are exported.
// 2. User no loger has option to export x-axis as pixels; x-axis is exported in units.
...
// get the root name from the front-most data figure

image front := getfrontimage()
string parentfilename = getname(front)
hideimage(front)
		...
		if(ysize==1) // It is a 1D profile
			{
				...
				imgname = parentfilename + imgname
