...
data_dir = 'C:/Users/Besitzer/Dropbox/WillB/Crozier_Lab/Writing/15_WJB_gb misorientation OIM EELS/figures/gpdc-gb-concentration/'
...
sp0_entries = ( '$Ce_{GB}$', '$Ce_{Gr.}$', '$Gd_{GB}$', '$Gd_{Gr.}$', 
    '$Pr_{GB}$', '$Pr_{Gr.}$' ) # legend
...
sp1_x_lab, sp1_y_lab = 'Concentration (Mole %)', 'Length fraction'
...
pl.figure( figsize = ( 3.5, 6 ) ) # create a figure of size ( width", height" )
...
ce_gb_fit = -0.0061 * angle + 0.88
gd_gb_fit = 0.0038 * angle + 0.093
pr_gb_fit = 0.0021 * angle + 0.036
# ce_gb_fit = 1 - ( gd_gb_fit + pr_gb_fit )
pl.plot( angle, ce_gb_fit, ls='-', c=ce_c )
pl.plot( angle, gd_gb_fit, ls='-', c=gd_c )
pl.plot( angle, pr_gb_fit, ls='-', c=pr_c )
...
for dot in dots:
    ...
    pl.savefig( output_name+'.png', format=file_type, dpi=dot, transparent=True )
