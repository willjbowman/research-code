...
fig_name = 'PCO-HRTEM-grain-size'
fig_dir = paper_dir + 'figures/' + fig_name + '/'
data_dir = paper_dir + 'data/' + fig_name + '/'
d_in = data_dir + 'PCO-HRTEM-grain-size.txt'
...
leg_loc = 'upper right'
x_lab = r'Anneal temp. ($^{\bigcirc}C$)' #'${2\Theta}$ (Degrees)'
y0_lab, y1_lab = 'Mean grain size (nm)', ''
# naming sequence of figs with successive curves on same axis
# file_anno = [ '-0of1', '-1of1' ]
file_anno = [ '-0of0' ] # for single fig with all curves
x_lims, y0_lims, y1_lims = [600, 900], [0, 20], [.1, .6]
...
# if len( file_anno ) == 0:
    ...
    pl.errorbar( d_T, d_dg, yerr=d_dg_std, c=cols[0], fmt='o', capthick=1, markersize=5 )
